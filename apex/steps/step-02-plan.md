...
{if tdd_mode:}
**TDD execution order:** For every acceptance criterion, specify the focused test file and behavior first, then the minimum production change. The implementation step must run that test failing before code and passing after code; refactoring follows only while it remains green.
