...
auto_mode: true # Default; --no-auto restores confirmation pauses
examine_mode: false # -x: Auto-proceed to adversarial review
save_mode: false # -s: Save outputs to .claude/output/apex/
test_mode: true # Required by the default TDD workflow
tdd_mode: true # Default; --no-tdd opts out and restores the classic test flow
...
Enable flags (lowercase - turn ON):
  ...
  -d or --tdd      → {tdd_mode} = true, {test_mode} = true
...
**Reject contradictory flags before any repository action:** `-a` with `-A`, `-d` with `-D`,
or any other ON/OFF pair is an input error. Ask for one unambiguous choice; do not silently
resolve it by argument order.
