...
| `{tdd_mode}` | Test-drive each behavior |
...
**If `{tdd_mode}` = false:** Convert each file change from the plan into todos:
...
**If `{tdd_mode}` = true:** Convert each acceptance criterion into one behavior todo. Each todo must name its focused test and minimum production files. Do not create a separate, post-implementation testing todo.

### 3. Execute File by File

For each todo:

**If `{tdd_mode}` = true, use this loop instead of sections 3.2–3.3:**

1. Read the comparable test and production files named in the plan.
2. Write one focused behavioral test for the current acceptance criterion.
3. Run the narrowest relevant test command and confirm it fails for the intended missing behavior (RED). A setup, syntax, or unrelated failure is a blocker, not a passing RED step.
4. Implement only the minimum production change needed for that test.
5. Re-run the focused test and confirm it passes (GREEN).
6. Refactor only if it improves the implementation; rerun the focused test after every refactor and keep it green.

Do not start the next behavior until the current behavior is green.
...
✅ In TDD mode, every acceptance criterion has an observed RED and GREEN cycle
