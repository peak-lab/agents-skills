...
| `{tdd_mode}` | Test-drive each assigned behavior |
...
Task:
  ...
  prompt: |
    ...
    **Task:** {task-id} - {task-title}
    ...
    ### TDD Protocol
    If `{tdd_mode}` is true, split your assignment by acceptance criterion. For each one: read comparable tests, add a focused behavioral test, run it and confirm the intended failure, implement the minimum change, run it green, then refactor only while it stays green. Report the RED and GREEN commands/results with your completion message.
...
✅ TDD assignments report a focused RED and GREEN cycle for each behavior
