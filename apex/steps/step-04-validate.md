...
| `{tdd_mode}` | Tests were created during execution |
...
```
IF {tdd_mode} = true AND {examine_mode} = true:
    → Load step-05-examine.md (tests were created during execution)

ELSE IF {tdd_mode} = true AND {pr_mode} = true:
    → Load step-09-finish.md

ELSE IF {tdd_mode} = true:
    → Complete workflow (tests were created during execution)
...
- **If tdd_mode and examine_mode:** Load `./step-05-examine.md`
- **If tdd_mode and pr_mode:** Load `./step-09-finish.md`
- **If tdd_mode:** Workflow complete; do not load post-implementation test steps
