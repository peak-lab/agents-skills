...
| TDD (`-d`) | {tdd_mode ? "✓ ON" : "✗ OFF"} | Red, green, refactor during implementation |
...
questions:
  - header: "Configure"
    ...
    options:
      ...
      - label: "TDD mode"
        description: "{tdd_mode ? 'Disable' : 'Enable'} - test-drive each behavior"
    multiSelect: true
```
...
IF "TDD mode" selected → {tdd_mode} = !{tdd_mode}
...
If TDD mode is enabled, keep test mode enabled:
```
IF {tdd_mode} = true → {test_mode} = true
```
