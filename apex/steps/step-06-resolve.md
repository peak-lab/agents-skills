...
**If `{tdd_mode}` = true:** Re-run the focused tests covering every resolved finding, then run the full test command discovered during validation. If context restoration lost that command, rediscover it using the Step 04 validation rules first. Do not complete resolution until both are green.
...
✅ In TDD mode, focused and full tests pass after review fixes
